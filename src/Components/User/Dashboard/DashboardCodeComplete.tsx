...
function DashboardCodeComplete() {
  const [solutions, setSolutions] = useState<ICode_Complete_Solution[]>([]);
  ...
  useEffect(() => {
    const fetchSolutionsData = async () => {
      ...
      try {
        ...
        >(`/solutions/code-complete/user/${userContext.id}`);
    ...
    const fetchChallengesNumber = async () => {
      ...
      try {
        ...
        if (!data.result) {
          setAppContext(prevContext => ({ ...prevContext, isLoading: false }));
          toast.error('Erro ao procurar desafios Code_Complete');
          return;
        }
...
export default DashboardCodeComplete;
