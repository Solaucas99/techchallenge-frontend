  ...
  useEffect(() => {
    const fetchSolutionsData = async () => {
      ...
      try {
        const { data } = await instanceAPI.get<IAxiosGet<IHTML_Solution[]>>(
          `/solutions/html/user/${userContext.id}`
        );
    ...
    const fetchChallengesNumber = async () => {
      ...
      try {
        ...
        if (!data.result) {
          setAppContext(prevContext => ({ ...prevContext, isLoading: false }));
          toast.error('Erro ao procurar desafios HTML');
          return;
        }
          ...
          <SolutionsList>
            {solutions.length > 0 &&
              solutions.map(solution => (
                <li key={solution.id}>
                  <h3>{solution.HTML_Challenge.title}</h3>
                  <br />
                  <CodeMirrorSolution
                  ...
                  <br />
                  <Link
                    to={`/challenges/categories/html/${solution.HTML_Challenge.id}/solutions`}
