...
function CodeCompleteChallengesListIndex() {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [rows, setRows] = useState<ICode_Complete_Challenge[]>([]);
      ...
      if (value) {
        try {
          ...
          const removedChallenge = await instanceAPI.delete<
            IAxiosDelete<ICode_Complete_Challenge>
          >(`/challenges/code-complete/delete/${challengeId.current}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await instanceAPI.get<
          IAxiosGet<ICode_Complete_Challenge[]>
        >(`/challenges/code-complete/uncompleted/${userContext.id}`);
  ...
  return (
    <Container>
      ...
      />
      <ContainerContent>
        <PageTitleH1>Desafios de Code Complete</PageTitleH1>
        ...
        {userContext.is_admin && (
          <Link
            style={{ color: 'inherit', textDecoration: 'none' }}
            to="/admin/challenges/categories/code-complete/create"
          >
            <Button
              variant="outlined"
              type="button"
...
export default CodeCompleteChallengesListIndex;
