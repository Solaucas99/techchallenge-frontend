...
function QuizChallengesListIndex() {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [rows, setRows] = useState<IJS_Challenge[]>([]);
      ...
      if (value) {
        try {
          ...
          const removedChallenge = await instanceAPI.delete<
            IAxiosDelete<IQuiz_Challenge>
          >(`/challenges/quiz/delete/${challengeId.current}`);
    ...
    const fetchData = async () => {
      try {
        ...
        if (userContext.is_admin) {
          const { data } = await instanceAPI.get<IAxiosGet<IQuiz_Challenge[]>>(
            `/challenges/quiz/`
          );
        ...
        const { data } = await instanceAPI.get<IAxiosGet<IQuiz_Challenge[]>>(
          `/challenges/quiz/uncompleted/${userContext.id}`
        );
  ...
  return (
    <Container>
      ...
      />
      <ContainerContent>
        <PageTitleH1>Desafios de Quiz</PageTitleH1>
        ...
        {userContext.is_admin && (
          <Link
            style={{ color: 'inherit', textDecoration: 'none' }}
            to="/admin/challenges/categories/quiz/create"
          >
            <Button
              variant="outlined"
              type="button"
...
export default QuizChallengesListIndex;
