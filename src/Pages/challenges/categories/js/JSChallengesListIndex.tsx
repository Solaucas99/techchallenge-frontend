...
function JSChallengesListIndex() {
      ...
      if (value) {
        try {
          ...
          const removedChallenge = await instanceAPI.delete<
            IAxiosDelete<IJS_Challenge>
          >(`/challenges/js/delete/${challengeId.current}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await instanceAPI.get<IAxiosGet<IJS_Challenge[]>>(
          `/challenges/js/uncompleted/${userContext.id}`
        );
  ...
  return (
    <Container>
      ...
      />
      <ContainerContent>
        <PageTitleH1>Desafios JavaScript</PageTitleH1>
        ...
        {userContext.is_admin && (
          <Link
            style={{ color: 'inherit', textDecoration: 'none' }}
            to="/admin/challenges/categories/js/create"
          >
            <Button
              variant="outlined"
              type="button"
...
export default JSChallengesListIndex;
